...
import { useEditAutor } from "../../hooks/Autores/useEditAutor";

// Definido fuera para evitar pérdida de foco al escribir
...
export default function AutorEdit({ isOpen, onClose, onRefresh, autorId }) {
  const { formData, estados, loading, handleChange, handleUpdate } = useEditAutor(autorId, onRefresh, onClose);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/20 backdrop-blur-sm animate-in fade-in duration-300 p-4">
      <div className="bg-slate-200 w-full max-w-md p-8 rounded-[2.5rem] shadow-[20px_20px_60px_#bebebe,-20px_-20px_60px_#ffffff] relative max-h-[90vh] overflow-y-auto custom-scrollbar">
        ...
        <h2 className="text-2xl font-light text-slate-800 mb-8">
          Editar <span className="font-semibold text-blue-600">Autor</span>
        </h2>

        <form onSubmit={handleUpdate} className="space-y-5">
          <NeumorphicField icon={User} name="primerNombre" placeholder="Nombre" value={formData.primerNombre} onChange={handleChange} />
          <NeumorphicField icon={User} name="primerApellido" placeholder="Apellido" value={formData.primerApellido} onChange={handleChange} />
          <NeumorphicField icon={User} name="segundoApellido" placeholder="Segundo Apellido" required={false} value={formData.segundoApellido} onChange={handleChange} />
          <NeumorphicField icon={Globe} name="nacionalidad" placeholder="Nacionalidad" value={formData.nacionalidad} onChange={handleChange} />
          ...
          <div className="relative">
            ...
            >
              ...
              {estados.map(est => (
                <option key={est.estadoID} value={est.estadoID}>{est.nombreEstado}</option>
              ))}
            </select>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full flex items-center justify-center gap-2 py-4 mt-4 bg-slate-200 rounded-2xl shadow-[6px_6px_12px_#c8d0e7,-6px_-6px_12px_#ffffff] hover:shadow-[inset_4px_4px_8px_#c8d0e7,inset_-4px_-4px_8px_#ffffff] text-blue-600 font-bold transition-all disabled:opacity-50 active:scale-95"
          >
