        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="text-[11px] uppercase tracking-[0.2em] font-bold text-slate-500">
              ...
              <th className="px-6 py-5">Nacionalidad</th>
              ...
              autores.map((a) => (
                <tr key={a.autorID} className="group hover:bg-white/30 transition-colors">
                  <td className="px-8 py-4 flex items-center gap-3">
                    ...
                    <span className="font-medium text-slate-700">{a.primerNombre} {a.primerApellido}</span>
                  </td>
                  <td className="px-6 py-4 text-slate-600 text-sm italic">
                    <Globe size={14} className="inline mr-2 opacity-50" />
                    {a.nacionalidad}
                  </td>
                  <td className="px-6 py-4 text-center">{renderBadge(a.nombreEstado)}</td>
                  <td className="px-8 py-4 text-right">
                    <button 
                      onClick={() => handleEdit(a.autorID)}
