...
export default function CategoriaCreate({ isOpen, onClose, onRefresh }) {
  const { formData, loading, handleChange, saveCategoria } =
    useCreateCategoria(onRefresh, onClose);
    ...
    bg-slate-900/20 backdrop-blur-sm animate-in fade-in duration-300 p-4">
      ...
      relative">
        ...
        <h2 className="text-2xl font-light text-slate-800 mb-8">
          Nueva <span className="font-semibold text-blue-600">Categoría</span>
        </h2>

        <form onSubmit={saveCategoria} className="space-y-6">
