...
import { useCreateLibro } from "../../hooks/Libro/useCreateLibro";
...
export default function LibroCreate({ isOpen, onClose, onRefresh }) {
  ...
  const {
    ...
    handleChange,
    saveLibro
  } = useCreateLibro(onRefresh, onClose);
    ...
    bg-slate-900/20 backdrop-blur-sm animate-in fade-in duration-300 p-4">
      ...
      relative max-h-[90vh] overflow-y-auto custom-scrollbar">
        ...
        <h2 className="text-2xl font-light text-slate-800 mb-8">
          Registrar <span className="font-semibold text-blue-600">Nuevo Libro</span>
        </h2>

        <form onSubmit={saveLibro} className="space-y-5">
          ...
          >
            {loading ? "Guardando..." : <><Save size={18} /> Guardar Libro</>}
          </button>
