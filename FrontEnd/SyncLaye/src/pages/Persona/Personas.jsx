...
import { Plus, Edit2, Mail, Contact, ArrowLeft, Fingerprint, UserCircle } from "lucide-react";
import { Link } from "react-router-dom";
import PersonaCreate from "./PersonaCreate";
import PersonaEdit from "./PersonaEdit";

export default function Personas() {
  const { personas, loading, refetch } = usePersonas();
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto animate-in fade-in duration-500">
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {loading ? (
              ...
              personas.map((p) => (
                <tr key={p.personaID} className="group hover:bg-white/30 transition-colors">
      ...
      <PersonaCreate isOpen={ui.isCreateOpen} onClose={closeCreate} onRefresh={refetch} />
      <PersonaEdit isOpen={ui.isEditOpen} onClose={closeEdit} onRefresh={refetch} personaId={ui.selectedId} />
    </div>
  );
}